...
async function waitForServer(url, timeoutMs = 45000) {
  ...
  throw new Error(`Dev server no respondió en ${timeoutMs}ms`);
}
...
async function runTest() {
  ...
  try {
    ...
    // Verificar que el auto-format de fecha funcionó
    ...
    if (REAL_SEND) {
      ...
      ok('TEST OK — formulario funciona end-to-end (sin enviar email real)');
    }
  } catch (err) {
    fail(err.message);
    if (devLog.length) {
